...
export default function Home() {
  ...
  return (
    <main>
      ...
      <section className="category-strip"><div className="section-heading"><span>✦</span><h2>Shop by Category</h2><span>✦</span></div><div className="category-nav">{["All","Everyday","Bottom Wear","Dresses","Co-ords","Party Wear"].map((c) => <button className={activeCategory === c ? "active" : ""} key={c} onClick={() => setActiveCategory(c)}>{c}</button>)}</div></section>

      <section className="shop" id="shop"><div className="shop-heading"><div><span className="eyebrow">THE CARNIVAL EDIT</span><h2>Pieces to <i>fall for</i></h2></div><a className="text-link" href="#shop">Shop all&nbsp; →</a></div><div className="product-grid">{shownProducts.map((p) => <article className="product-card" key={p.name}><div className="product-image">{p.badge && <em>{p.badge}</em>}<img src={p.img} alt={p.name} style={{objectPosition: `${p.position || "50%"} center`}}/><button aria-label="Add to wishlist">♡</button></div><div className="product-info"><div><h3>{p.name}</h3><p>{p.type}</p></div><strong>{p.price} {p.old && <del>{p.old}</del>}</strong></div><button className="add-button" onClick={() => addToBag(p.name)}>Add to bag <span>+</span></button></article>)}</div></section>
