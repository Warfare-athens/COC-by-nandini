...
export function FeedbackForm() {
  ...
  return <form className="grid max-w-2xl gap-4" onSubmit={submit}><div className="grid gap-4 sm:grid-cols-2"><input className={field} name="name" placeholder="Name"/><input className={field} name="email" type="email" placeholder="Email"/><select className={field} name="type"><option>general</option><option>product</option><option>delivery</option><option>website</option></select><select className={field} name="rating"><option value="">No rating</option>{[5,4,3,2,1].map(v=><option key={v} value={v}>{v}/5</option>)}</select></div><textarea className={`${field} min-h-32`} name="message" placeholder="Your feedback" required minLength={5}/><button className="w-max rounded-full bg-[#bb7068] px-6 py-3 text-xs font-semibold uppercase tracking-[.1em] text-white" disabled={busy}>{busy ? "Sending…" : "Send feedback"}</button></form>;
}
