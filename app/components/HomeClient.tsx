...
import { NewsletterForm } from "./EngagementForms";
import { HomeProduct, mapDbProductToHomeProduct } from "@/lib/product-mapper";
import SizePickerPopover from "./SizePickerPopover";
...
export default function HomeClient({ initialProducts = [] }: { initialProducts?: HomeProduct[] }) {
  ...
  useEffect(() => {
    ...
    fetch("/api/storefront/products", { cache: "no-store" })
      ...
      .then((payload) => {
        ...
        setProducts(payload.products.map(mapDbProductToHomeProduct));
      })
      .catch(() => setProducts([]));
    return () => {
      cancelled = true;
  ...
  return (
    <main ref={pageRef}>
      ...
      <section className="shop category-strip" id="shop">
        <div id="categories" style={{ position: "relative", top: "-100px" }} />
        ...
        <div className="shop-heading" style={{ justifyContent: "center", textAlign: "center", margin: "14px auto 30px" }}>
          <div>
            <span className="eyebrow" style={{ display: "block", marginBottom: "4px" }}>
              THE CARNIVAL EDIT · {shownProducts.length} {shownProducts.length === 1 ? "PIECE" : "PIECES"}
            </span>
            <h3 className="letter-heading" style={{ fontSize: "32px", margin: 0 }} aria-label="Pieces to fall for">
        ...
        <div className="category-nav" style={{ justifyContent: "center", gap: "10px", flexWrap: "wrap", marginBottom: "35px" }}>
        ...
        {activeCategory !== "All" && (
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: "10px", marginBottom: "28px", fontSize: "12px" }}>
            <span style={{ color: "#7a625a" }}>Filtered by: <b>{activeCategory}</b> ({shownProducts.length} {shownProducts.length === 1 ? "piece" : "pieces"})</span>
            <button
              type="button"
              onClick={() => setActiveCategory("All")}
              style={{
                color: "var(--rose, #bb7068)",
                fontWeight: 600,
        ...
        <div style={{ textAlign: "center", marginTop: "48px" }}>
          <a className="category-all" href="/shop" style={{ display: "inline-block" }}>
