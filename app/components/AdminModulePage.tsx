...
import { AdminEmptyState, AdminPageHeader, AdminPanel, AdminStatCard } from "@/app/components/AdminUI";
import { commerceConfigured, getSupabaseAdmin } from "@/db";
import { BLOG_POSTS } from "@/lib/blogs";
import { COLLECTIONS } from "@/lib/collections";

export type AdminModule = "ai-visibility" | "content" | "images" | "reviews" | "templates" | "partnerships" | "invoices" | "tracking" | "stock-requests" | "checkouts" | "cart-leads" | "coupon-leads" | "feedback" | "customers" | "login-activity" | "data-export";
...
function Head({ title, action }: { title: string; action?: React.ReactNode }) { const live = ["Checkouts", "Cart Leads"].includes(title); return <AdminPageHeader title={title} actions={action || (live ? <AdminLiveRefresh/> : null)} />; }
function Kpis({ items }: { items: [string, React.ReactNode][] }) { return <div className="admin-kpis">{items.map(([label, value]) => <AdminStatCard label={label} value={value} key={label} />)}</div>; }
function Empty({ label = "No records yet." }: { label?: string }) { return <AdminEmptyState title={label} />; }
function Panel({ title, children }: { title: string; children: React.ReactNode }) { return <AdminPanel title={title}>{children}</AdminPanel>; }
...
export default async function AdminModulePage({ module }: { module: AdminModule }) {
  ...
  if (module === "ai-visibility") {
    ...
    return <><Head title="AI Visibility"/><Kpis items={[["Visibility score", `${score}%`], ["Indexable products", active.length], ["Journal guides", BLOG_POSTS.length], ["Collections", COLLECTIONS.length]]}/><div className="admin-detail-grid"><Panel title="Search readiness"><div className="admin-summary-list"><div><span>Product metadata complete</span><b>{complete}/{active.length}</b></div><div><span>Product sitemap</span><b>Automatic</b></div><div><span>Structured product data</span><b>Enabled</b></div><div><span>Brand entity schema</span><b>Enabled</b></div><div><span>AI crawler guide</span><a href="/llms.txt" target="_blank">Open ↗</a></div><div><span>RSS feed</span><a href="/blog/rss.xml" target="_blank">Open ↗</a></div></div></Panel><Panel title="Coverage"><div style={{padding:22,display:"grid",placeItems:"center",gap:16}}><div className="admin-score">{score}</div><div className="admin-progress" style={{width:"100%"}}><span style={{width:`${score}%`}}/></div><a className="admin-small-button" href="/sitemap.xml" target="_blank">Open sitemap</a></div></Panel></div></>;
  }
  ...
  if (module === "images") {
    ...
    return <><Head title="Images"/><Kpis items={[["Library", assets.length], ["Product images", productImages.length], ["Total assets", all.length], ["Missing alt text", all.filter(r=>!r.alt_text).length]]}/><Panel title="Upload"><AdminCreateForm module="media"/></Panel><Panel title="Image library">{all.length?<div className="admin-media-grid">{all.map((row,index)=><article className="admin-media-tile" key={`${text(row.id)}-${index}`}><img src={text(row.url)} alt={text(row.alt_text || "Carnival of Clothes image")}/><div><small>{text(row.alt_text || row.filename || "No alt text")}</small><small>{text(row.source)} · {text(row.folder || "products")}</small></div></article>)}</div>:<Empty/>}</Panel></>;
  }
      ...
      <>
        ...
        <Kpis
          items={[
            ...
            ["Total Billed", money(totalPaidBilled)],
          ]}
        />
          ...
          {invoices.length ? (
            <div className="admin-table-wrap">
              <table className="admin-table">
                ...
                <tbody>
                  {invoices.map((row) => {
                    const addr = (row.addresses || {}) as Row;
                    const customerName = text(addr.full_name || row.email);
                    const isPaid = row.payment_status === "paid";
                    return (
                      <tr key={text(row.id)}>
                        <td>
                          <b>INV-{text(row.order_number)}</b>
                          <small className="admin-row-sub">{date(row.placed_at)}</small>
                        </td>
                        <td>
                          <b>{customerName}</b>
                          <small className="admin-row-sub">
                            {text(row.email)} {row.phone ? `· ${text(row.phone)}` : ""}
                          </small>
                        </td>
                        <td>
                          <b>{money(row.total_inr)}</b>
                        </td>
                        <td>
                          <span className={`admin-status ${isPaid ? "good" : "warn"}`}>
                        ...
                        <td>
                          <a
                            ...
                            href={`/admin/invoices/${text(row.id)}`}
                            target="_blank"
            ...
            <Empty label="No invoices yet. Invoices are automatically generated when orders are placed." />
          )}
        </Panel>
      </>
    );
  ...
  if (module === "checkouts") {
    ...
    return <><Head title="Checkouts"/><Kpis items={[["Completed",orders.length],["Live checkout drafts",live.length],["Dropped at payment",live.filter(r=>r.checkout_step==="payment").length],["Revenue",money(orders.reduce((sum,r)=>sum+Number(r.total_inr||0),0))]]}/><Panel title="Live checkout journeys">{live.length?<div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Customer</th><th>Progress</th><th>Source</th><th>Status</th><th>Last activity</th><th>Journey</th></tr></thead><tbody>{live.map(row=><tr key={text(row.id)}><td><a className="admin-table-link" href={`/admin/checkouts/${text(row.id)}`}>{text(row.full_name)}</a><small className="admin-row-sub">{text(row.email)} · {text(row.phone)}</small></td><td><span className="admin-status warn">{text(row.checkout_step || "cart")}</span></td><td><b>{text(row.source || "direct")}</b><small className="admin-row-sub">{text(row.medium)} · {text(row.device)}</small></td><td><span className={`admin-status ${statusClass(row.status)}`}>{text(row.status)}</span></td><td>{date(row.last_activity_at)}</td><td><div style={{ display: "flex", alignItems: "center", gap: "6px" }}><a className="admin-small-button" href={`/admin/checkouts/${text(row.id)}`}>View journey</a><WhatsAppRecoveryAction cartId={text(row.id)} phone={text(row.phone)} customerName={text(row.full_name)} items={((row.cart_items || []) as Row[]).map((i) => { const rel = i.products as Row | Row[] | null; const p = Array.isArray(rel) ? rel[0] : rel; return { name: text(p?.name), quantity: Number(i.quantity || 1) }; })} currentStatus={text(row.status)} variant="compact"/></div></td></tr>)}</tbody></table></div>:<Empty label="No checkout drafts yet."/>}</Panel><Panel title="Recent completed checkouts">{orders.length?<div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Order</th><th>Customer</th><th>Total</th><th>Payment</th><th>Open</th></tr></thead><tbody>{orders.map(row=>{const linkedCart=cartForOrder.get(text(row.id));return <tr key={text(row.id)}><td><b>{text(row.order_number)}</b><small className="admin-row-sub">{date(row.placed_at)}</small></td><td>{text(row.email)}</td><td>{money(row.total_inr)}</td><td><span className={`admin-status ${statusClass(row.payment_status)}`}>{text(row.payment_status)}</span></td><td><a className="admin-small-button" href={linkedCart?`/admin/checkouts/${text(linkedCart.id)}`:`/admin/orders/${text(row.id)}`}>{linkedCart?"View journey":"Order details"}</a></td></tr>})}</tbody></table></div>:<Empty/>}</Panel></>;
  }
  ...
  if (module === "login-activity") {
    ...
    return <><Head title="Login Activity"/><Kpis items={[["Sessions logged",sessions.length],["Logins",sessions.filter(r=>r.action==="session.login").length],["Logouts",sessions.filter(r=>r.action==="session.logout").length],["All admin actions",activity.length]]}/><Panel title="Security activity">{activity.length?<div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Action</th><th>Actor</th><th>Record</th><th>Date</th></tr></thead><tbody>{activity.map(row=><tr key={text(row.id)}><td><b>{text(row.action)}</b></td><td>{text(row.actor)}</td><td>{text(row.entity_type)}<small className="admin-row-sub">{text(row.entity_id)}</small></td><td>{date(row.created_at)}</td></tr>)}</tbody></table></div>:<Empty/>}</Panel></>;
  }
  ...
  return <><Head title="Data Export"/><Kpis items={[["Export sets",exports.length],["Format","CSV"],["Access","Admin only"],["Generated","On demand"]]}/><Panel title="Download data"><div className="admin-media-grid">{exports.map(([label,type])=><a className="admin-card" href={`/api/admin/export/${type}`} key={type}><span>{label}</span><strong style={{fontSize:22}}>Download CSV ↓</strong></a>)}</div></Panel></>;
}
