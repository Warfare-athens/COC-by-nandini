...
const schema = z.object({ announcementEnabled: z.boolean(), announcementMessage: z.string().max(160), announcementLink: z.string().max(300).optional(), freeShippingThreshold: z.number().int().min(0), shippingCharge: z.number().int().min(0), codEnabled: z.boolean(), maintenanceMode: z.boolean(), razorpayEnabled: z.boolean().default(true) });
export async function POST(request: Request) {
  ...
  try {
    ...
    const values = { free_shipping_threshold: input.freeShippingThreshold, shipping_charge: input.shippingCharge, cod_enabled: input.codEnabled, maintenance_mode: input.maintenanceMode, razorpay_enabled: input.razorpayEnabled };
