...
function Head({ title, action }: { title: string; action?: React.ReactNode }) { return <div className="admin-head"><h1>{title}</h1>{action}</div>; }
...
export default async function AdminModulePage({ params }: { params: Promise<{ module: string }> }) {
  ...
  if (module === "checkouts") {
    const [orders,carts] = await Promise.all([rows("orders", "id,order_number,email,total_inr,status,payment_status,fulfillment_status,placed_at", "placed_at"),rows("carts", "id,email,phone,status,last_activity_at,created_at,cart_items(id,quantity)", "last_activity_at")]);
    return <AdminShell><Head title="Checkouts"/><Kpis items={[["Completed",orders.length],["Active carts",carts.filter(r=>r.status==="active").length],["Abandoned",carts.filter(r=>r.status==="abandoned").length],["Revenue",money(orders.reduce((sum,r)=>sum+Number(r.total_inr||0),0))]]}/><Panel title="Recent completed checkouts">{orders.length?<div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Order</th><th>Customer</th><th>Total</th><th>Payment</th><th>Open</th></tr></thead><tbody>{orders.map(row=><tr key={text(row.id)}><td><b>{text(row.order_number)}</b><small className="admin-row-sub">{date(row.placed_at)}</small></td><td>{text(row.email)}</td><td>{money(row.total_inr)}</td><td><span className={`admin-status ${statusClass(row.payment_status)}`}>{text(row.payment_status)}</span></td><td><a className="admin-small-button" href={`/admin/orders/${text(row.id)}`}>Details</a></td></tr>)}</tbody></table></div>:<Empty/>}</Panel></AdminShell>;
  }

  if (module === "cart-leads") {
    const [carts, cartEvents] = await Promise.all([
      rows("carts", "id,anonymous_token,email,phone,status,last_activity_at,created_at,cart_items(id,quantity,products(name,price_inr))", "last_activity_at", 1000),
    ...
    const isStale = (row: Row) => row.status === "active" && new Date(String(row.last_activity_at)).getTime() <= staleBoundary;
    ...
    const abandoned = carts.filter(row => row.status === "abandoned" || isStale(row));
    ...
    return <AdminShell><Head title="Cart Leads"/><Kpis items={[["Tracked carts",carts.length],["Contactable",contactable.length],["Abandoned / stale",abandoned.length],["Recoverable value",money(recoverableValue)]]}/><Panel title="Cart activity">{carts.length?<div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Lead</th><th>Contact</th><th>Items</th><th>Value</th><th>Status</th><th>Last activity</th><th>Follow up</th></tr></thead><tbody>{carts.map(row=>{const items=(row.cart_items||[]) as Row[];const displayStatus=isStale(row)?"stale":text(row.status);return <tr key={text(row.id)}><td><b>{text(row.anonymous_token).slice(0,12)}</b><small className="admin-row-sub">{items.map(item=>{const relation=item.products as Row|Row[]|null;const product=Array.isArray(relation)?relation[0]:relation;return text(product?.name)}).filter(name=>name!=="—").join(", ")||"No items"}</small></td><td><b>{text(row.email)}</b><small className="admin-row-sub">{text(row.phone)}</small></td><td>{items.reduce((sum,item)=>sum+Number(item.quantity||0),0)}</td><td><b>{money(value(row))}</b></td><td><span className={`admin-status ${displayStatus==="stale"?"warn":statusClass(displayStatus)}`}>{displayStatus}</span></td><td>{date(row.last_activity_at)}</td><td><AdminRecordAction module="carts" id={text(row.id)} current={text(row.status)} options={["active","contacted","recovered","abandoned","closed"]}/></td></tr>})}</tbody></table></div>:<Empty label="No server-side carts yet."/>}</Panel><Panel title="Tracking health"><div className="admin-summary-list"><div><span>Add-to-cart events</span><b>{cartEvents.filter(row=>row.event_name==="add_to_cart").length}</b></div><div><span>Cart views</span><b>{cartEvents.filter(row=>row.event_name==="cart_viewed").length}</b></div><div><span>Remove / quantity events</span><b>{cartEvents.filter(row=>["remove_from_cart","cart_quantity_changed"].includes(String(row.event_name))).length}</b></div><div><span>Checkout starts</span><b>{new Set(cartEvents.filter(row=>row.event_name==="checkout_started").map(row=>row.anonymous_id)).size}</b></div><div><span>Captured checkout contacts</span><b>{new Set(cartEvents.filter(row=>row.event_name==="checkout_contact_captured").map(row=>row.anonymous_id)).size}</b></div><div><span>Checkout errors</span><b>{cartEvents.filter(row=>row.event_name==="checkout_failed").length}</b></div></div></Panel></AdminShell>;
  }
